...
class AirSimPhysicsAdapter(SimAdapter):
    ...
    def set_velocity_body(self, forward: float, right: float, down: float,
        ...
        """
        Body frame 速度控制，主要用于 look_around 原地旋转。
        forward/right/down: m/s (暂不支持平移，仅 yaw_rate 有效)
        yaw_rate: deg/s
        用 moveByRollPitchYawrateZAsync 实现原地旋转。
        """
        ...
        try:
            ...
            # roll=0, pitch=0 保持悬停; yaw_rate 控制旋转; z 保持当前高度
    ...
    def _fly_to_with_rpyz(self, target_x: float, target_y: float, target_z: float,
        ...
        """
        MAX_PITCH = 0.22      # 最大前倾角（弧度，约 12.6°；减小以避免上下摆头）
        ...
        Z_SMOOTH = 0.3        # z 高度平滑系数（0~1, 越小越平滑）
        ...
        try:
            while True:
                ...
                # 发送飞行指令（z 用平滑插值，避免 pitch 耦合导致上下摆头）
                cmd_z = cz + Z_SMOOTH * (target_z - cz)
                try:
                    self._fly_client.move_by_roll_pitch_yaw_z(
                        0.0, pitch, yaw, cmd_z, CMD_DURATION, self._vehicle_name
                    )
                except Exception as e:
    ...
    def connect(self, connection_str: str = "", timeout: float = 15.0) -> bool:
        ...
        try:
            ...
            # 传送到初始位置 50m 高空 (z=-50 in AirSim NED)
            INIT_ALT_Z = -50.0  # 50m 高空，永远记住这个值
            logger.info(f"Teleporting drone to initial position (0, 0, z={INIT_ALT_Z}) = 50m altitude...")
            try:
                init_pose = {
                    "position": {"x_val": 0.0, "y_val": 0.0, "z_val": INIT_ALT_Z},
                ...
                logger.warning(f"Initial teleport failed: {e}")
    ...
    def land(self) -> ActionResult:
        """手动控制降落：先快速下降到 3m，再缓慢着陆。"""
        if not self._connected:
            return ActionResult(success=False, message="Not connected")
        try:
            alt = self._get_altitude()
            logger.info(f"Land: controlled descent from {alt:.1f}m")

            FAST_SPEED = 0.6    # 高空快降 pitch 幅度（z 每轮变化量的系数）
            SLOW_ALT = 5.0      # 切换到慢降的高度
            SLOW_SPEED = 0.3    # 低空缓降系数
            GROUND_ALT = 0.5    # 判定落地的高度
            CMD_DUR = 0.4
            TIMEOUT = 60.0
            ...
            while time.time() - start_t < TIMEOUT:
                cx, cy, cz = self._get_xyz()
                cur_alt = self._get_altitude()

                if cur_alt < GROUND_ALT:
                    break

                # 计算目标 z：当前 z 往下移一步
                if cur_alt > SLOW_ALT:
                    step = FAST_SPEED * CMD_DUR * 8  # ~1.9m/tick → ~4.8 m/s 下降
                else:
                    step = SLOW_SPEED * CMD_DUR * 4  # ~0.48m/tick → ~1.2 m/s 缓降
                target_z = cz + step  # NED: z 增大 = 向下
            ...
            # 最终使用原生 land 确保完全着陆
            try:
                self._fly_client.land_async_join(timeout_sec=10.0,
                                                 vehicle_name=self._vehicle_name)
            except Exception:
                pass
